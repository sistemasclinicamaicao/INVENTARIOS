# Migración 023 — columnas Socrata INVIMA adicionales (29 columnas)
...
$sql = Join-Path $Root "backend\migrations\023_invima_socrata_columns.sql"

Write-Host "Aplicando migracion 023 (columnas INVIMA Socrata)..." -ForegroundColor Cyan
Get-Content $sql -Raw | docker exec -i clinica_erp_postgres psql -U $user -d $db
if ($LASTEXITCODE -eq 0) {
    Write-Host "[OK] Migracion 023 aplicada" -ForegroundColor Green
} else {
