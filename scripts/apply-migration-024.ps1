# Migración integraciones REST (consulta GET sin plantilla OC)
...
$sql = Join-Path $Root "backend\migrations\024_rest_query_integration.sql"

Write-Host "Aplicando migracion 024 (integraciones REST)..." -ForegroundColor Cyan
Get-Content $sql -Raw | docker exec -i clinica_erp_postgres psql -U $user -d $db
if ($LASTEXITCODE -eq 0) {
    Write-Host "[OK] Migracion 024 aplicada" -ForegroundColor Green
} else {
