# Migración integración Socrata MEDICAMENTOS POS (a7iv-sme8)
...
$sql = Join-Path $Root "backend\migrations\025_medicamentos_pos_integration.sql"

Write-Host "Aplicando migracion 025 (MEDICAMENTOS POS)..." -ForegroundColor Cyan
Get-Content $sql -Raw | docker exec -i clinica_erp_postgres psql -U $user -d $db
if ($LASTEXITCODE -eq 0) {
    Write-Host "[OK] Migracion 025 aplicada" -ForegroundColor Green
} else {
