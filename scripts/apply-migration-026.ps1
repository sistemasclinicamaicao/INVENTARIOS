# Migración tablas locales Medicamentos POS
...
$sql = Join-Path $Root "backend\migrations\026_medicamentos_pos_registros.sql"

Write-Host "Aplicando migracion 026 (medicamentos_pos_registros)..." -ForegroundColor Cyan
Get-Content $sql -Raw | docker exec -i clinica_erp_postgres psql -U $user -d $db
if ($LASTEXITCODE -eq 0) {
    Write-Host "[OK] Migracion 026 aplicada" -ForegroundColor Green
} else {
