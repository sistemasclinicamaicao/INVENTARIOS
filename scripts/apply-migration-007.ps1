# Migración líneas OC: precio, lote, vencimiento
...
$sql = Join-Path $Root "backend\migrations\007_po_lines_extended.sql"

Write-Host "Aplicando migracion 007 (PO lines)..." -ForegroundColor Cyan
Get-Content $sql -Raw | docker exec -i clinica_erp_postgres psql -U $user -d $db
if ($LASTEXITCODE -eq 0) {
    Write-Host "[OK] Migracion 007 aplicada" -ForegroundColor Green
} else {
