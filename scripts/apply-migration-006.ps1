# Migración tablas referencia INVIMA
...
$sql = Join-Path $Root "backend\migrations\006_invima_reference.sql"

Write-Host "Aplicando migracion 006 (INVIMA)..." -ForegroundColor Cyan
Get-Content $sql -Raw | docker exec -i clinica_erp_postgres psql -U $user -d $db
if ($LASTEXITCODE -eq 0) {
    Write-Host "[OK] Migracion 006 aplicada" -ForegroundColor Green
} else {
